...
import flwr_modif
from flwr_modif.common.typing import Config, GetParametersIns
from flwr_modif.proto.transport_pb2 import ClientMessage, Parameters, Scalar
from flwr_modif.server.fleet.grpc_bidi.grpc_bridge import ResWrapper
from flwr_modif.server.fleet.grpc_bidi.grpc_client_proxy import GrpcClientProxy
...
class GrpcClientProxyTestCase(unittest.TestCase):
    ...
    def test_get_parameters(self) -> None:
        ...
        # Execute
        value: flwr_modif.common.GetParametersRes = client.get_parameters(
    ...
    def test_fit(self) -> None:
        ...
        # Prepare
        client = GrpcClientProxy(cid="1", bridge=self.bridge_mock)
        parameters = flwr_modif.common.ndarrays_to_parameters([np.ones((2, 2))])
        ins: flwr_modif.common.FitIns = flwr_modif.common.FitIns(parameters, {})
        ...
        assert flwr_modif.common.parameters_to_ndarrays(fit_res.parameters) == []
    ...
    def test_evaluate(self) -> None:
        ...
        # Prepare
        client = GrpcClientProxy(cid="1", bridge=self.bridge_mock)
        parameters = flwr_modif.common.Parameters(tensors=[], tensor_type="np")
        evaluate_ins: flwr_modif.common.EvaluateIns = flwr_modif.common.EvaluateIns(parameters, {})
    ...
    def test_get_properties(self) -> None:
        ...
        ins: flwr_modif.common.GetPropertiesIns = flwr_modif.common.GetPropertiesIns(
            config=request_properties
        )

        # Execute
        value: flwr_modif.common.GetPropertiesRes = client.get_properties(ins, timeout=None)
